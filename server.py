...
def private_msg(msg, reciver_nickname):
    msg_turned_to_bytes = bytes(f'{reciver_nickname}  :  {msg}','utf-8')
...
def handle_msg(client):
    ...
    while True:
        try:
            ...
            message_striped_without_nickname = message[ message.index(':') +1 : ].strip(' ') #### this removes the username and colon leaving  /private jack xup 
            if message_striped_without_nickname.startswith('/'): 
                ...
                if msg_list[0] == '/private':
                    if msg_list[1] in client_nicknames:
                        ...
                        private_msg(senders_message,reciver_nickname)
                    else:
...
def verify(client_socket):
    ...
    while not nickname_exist:
            ...
            nickname_list_form = client_socket.recv(4096).decode('utf-8').strip(' ').split(" ")
            nickname = nickname_list_form[2]
